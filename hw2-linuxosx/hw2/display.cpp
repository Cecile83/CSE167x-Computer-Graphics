...
void display() 
{
  ...
  if (numused) {
    ...
    // Le problème est que j'ai plusieurs sources de lumière : il faut donc le faire en plusieurs fois
    
    GLfloat pos_aux[4], light_aux[4];
    
    for (int i=0 ; i < numused ; i++)
    {
      pos_aux[0] = lightposn[4*i+0];
      pos_aux[1] = lightposn[4*i+1];
      pos_aux[2] = lightposn[4*i+2];
      pos_aux[3] = lightposn[4*i+3];
	  
      transformvec(pos_aux, light_aux); 
      
      lightransf[4*i+0] = light_aux[0];
      lightransf[4*i+1] = light_aux[1];
      lightransf[4*i+2] = light_aux[2];
      lightransf[4*i+3] = light_aux[3];
  ...
  mat4 sc(1.0) , tr(1.0), transf(1.0), transf_obj(1.0); 
  ...
  for (int i = 0 ; i < numobjects ; i++) {
    ...
    transf_obj = transf * obj->transform ;
    glLoadMatrixf(&transf_obj[0][0]); 
